...
# Read positions
criegee = io.read("ch2oo-2.xyz", format='xyz')
...
write.coor_pdb("ch2oo-2.pdb", title="Criegee")
write.psf_card("ch2oo-2.psf", title="Criegee")
...
# Write pdb and psf files
write.coor_pdb("mini_ch2oo-2.pdb", title="Criegee optimized")

## Step 4: Set initial conditions
##-----------------------------------------------------------
#
stream.charmm_script("stream test-seed.str")
...
if True:
    ...
    res_file = pycharmm.CharmmFile(
        file_name='nve_seed.res', file_unit=1, formatted=True, read_only=False)
    dcd_file = pycharmm.CharmmFile(
        file_name='nve_seed.dcd', file_unit=2, formatted=False, read_only=False)
    vcd_file = pycharmm.CharmmFile(
        file_name='nve_seed.vcd', file_unit=3, formatted=False, read_only=False)
